...
int portNr=7824;
   ...
   */
   ...
   while (true) {
    try {nachricht = in.readUTF(); // auf die Antwort warten
     }
    ...
    {
     System.out.println(socket.getInetAddress().getHostName() +" : Ihre Meinung ist uns wichtig! Geben Sie bitte Ihre Antwort ein. Oder korrigieren Sie bitte Ihre Eingabe."); 
    }
    ...
    if (socket.isConnected()==false){System.out.println("Ende");break;}
    if (socket.isInputShutdown()){System.out.println("Ende");break;}
    if (socket.isOutputShutdown()){System.out.println("Ende");break;}
    if (socket.isBound()==false ){System.out.println("Ende");break;}
